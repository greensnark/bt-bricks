...
(function () {
  ...
  function renderImage(canvas) {
    ...
    img.addEventListener('load', function () {
      ...
      c.drawImage(img, 0, 0, 160, 35, 0, 0, 800, 175);
      setTimeout(function () {
        var c = canvas.getContext('2d');
        var idata = c.getImageData(0, 0, 800, 175);
        window.idata = idata;
        console.log("Image data at 87: R=" + (idata.data[87 * 800 * 4] === 0));
        window.blocks = traceBlocks(idata);
        drawBlocks(c, window.blocks);
      }, 0);
    });
    img.src = 'boxtone.png';

    var toggleBlock = function (x, y) {
      x = Math.floor(x);
      y = Math.floor(y);
      console.log("toggleBlock(" + x + ", " + y + ")");
      var blocks = window.blocks;
      for (var i = 0, length = blocks.length; i < length; ++i) {
        var b = blocks[i];
        if (b.x === x && b.y === y) {
          console.log("Found block match, removing it");
          blocks.splice(i, 1);
          return;
        }
      }
    ...
    canvas.addEventListener('click', function (e) {
      var rec = e.target.getClientRects()[0];
      var left = e.clientX - rec.left;
      var top = e.clientY - rec.top;
      console.log("click: (" + left + "," + top + ")");

      var blockLeft = Math.floor(left / (C.block.width + C.block.gutter)) * (C.block.width + C.block.gutter);
      var blockTop = Math.floor(top / (C.block.height + C.block.gutter)) * (C.block.height + C.block.gutter);
      toggleBlock(blockLeft, blockTop);
      var c = canvas.getContext('2d');
  ...
  function drawBlocks(c, blocks) {
    console.log("Drawing blocks!");
    var width = C.block.width, height = C.block.height;
    for (var i = 0, length = blocks.length; i < length; ++i) {
      var b = blocks[i];
      c.fillStyle = '#333';
      c.fillRect(b.x, b.y, width, height);
    }
  }

  function traceBlocks(idata) {
    var blockWidth = C.block.width;
    var blockHeight = C.block.height;
    var gutter = 1;
    var data = idata.data;
    ...
    var isLogo = function (x, y) {
      if (x >= width || y >= height) {
        return false;
      }
      var off = (Math.floor(y) * width + Math.floor(x)) * 4;
      var ored = data[off], ogreen = (data[off + 1] - 0x9d),
          oblue = data[off + 2] - 0xdc;
      return (ored * ored + ogreen * ogreen + oblue * oblue) < 10000;
    };

    var placeBlock = function (x, y) {
      blockList.push({x: x, y: y});
    };

    for (var y = 0; y < height; y += blockHeight + gutter) {
      for (var x = 0; x < width; x += blockWidth + gutter) {
        if (isLogo(x + blockWidth / 2, y + blockHeight / 2)) {
          placeBlock(x, y);
        }
      }
    }
    console.log("Found " + blockList.length + " blocks");
    return blockList;
  }

  initializeEgg(ecanvas('#easter'));
  renderImage(ecanvas('#easter'));
})();
